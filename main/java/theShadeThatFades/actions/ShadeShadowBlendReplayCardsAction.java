...
public class ShadeShadowBlendReplayCardsAction extends AbstractGameAction {
    ...
    private float max_deviation = 100.0f;
    ...
    public void update()
    {
        if (duration == Settings.ACTION_DUR_MED) {
            for (Map.Entry<AbstractCard, AbstractMonster> cardTargetPair : dupedCards.entrySet()) {
                ...
                tmp.current_x = tmp.target_x = Settings.WIDTH / 2.0f - 300.0f +(float)(Math.random() * max_deviation) * Settings.scale;
                tmp.current_y = tmp.target_y = Settings.HEIGHT / 2.0f +(float)(Math.random() * max_deviation);
